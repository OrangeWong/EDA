...
mice_plot <- aggr(labelled_subset, numbers=TRUE, 
                  sortVars=TRUE, labels=names(labelled_subset), 
                  cex.axis=0.725, gap = .0)
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
completed_data %>% 
  ggplot(aes(x = credit_grade, y = credit_score, colour = loan_stat)) +
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
completed_data %>% 
  ggplot(aes(x = credit_grade, y = APR, colour = loan_stat)) +
